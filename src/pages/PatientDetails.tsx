...
export default function PatientDetails() {
  ...
  const [examForm, setExamForm] = useState({ selectedCatalogId: "", nome: "", categoria: "laboratorial" as ExamCatalogItem["categoria"], descricaoCatalogo: "", data_exame: "", status: "", resultado: "", observacoes: "", pdf_nome: "" });
  ...
  const saveExam = async () => {
    if (!id || !examForm.data_exame || !examForm.status || !examForm.resultado) return;
    try {
      ...
      const payload = { nome, data_exame: examForm.data_exame, status: examForm.status, resultado: examForm.resultado, observacoes: examForm.observacoes || null, pdf_nome: examForm.pdf_nome || null, descricao: examMode === "new" ? examForm.descricaoCatalogo || null : null };
      ...
      setExamForm({ selectedCatalogId: "", nome: "", categoria: "laboratorial", descricaoCatalogo: "", data_exame: "", status: "", resultado: "", observacoes: "", pdf_nome: "" });
    } catch (error) {
  ...
  const saveMedication = async () => {
    if (!id || !medicationForm.periodo || !medicationForm.status || !medicationForm.descricao) return;
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="historico" className="space-y-4">
        ...
        <TabsContent value="exames"><Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" />Exames e resultados</CardTitle>{!secretaryMode && <Button variant="outline" onClick={() => setExamOpen(true)}><PlusCircle className="mr-2 h-4 w-4" />Adicionar</Button>}</CardHeader><CardContent className="space-y-3">{secretaryMode ? <p className="text-sm text-gray-500">O histórico de exames detalhado é acessível apenas ao médico.</p> : examRecords.length > 0 ? examRecords.map((exam) => <div key={exam.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-gray-900">{exam.name}</p><p className="text-sm text-gray-500">{new Date(exam.date).toLocaleDateString("pt-BR")}</p></div><div className="flex gap-2"><Badge variant="outline">{exam.status}</Badge><Button variant="outline" size="sm" onClick={() => { setEditingExamId(exam.id); setExamMode("existing"); setExamForm({ selectedCatalogId: "", nome: exam.name, categoria: "laboratorial", descricaoCatalogo: "", data_exame: exam.date, status: exam.status, resultado: exam.result, observacoes: exam.description || "", pdf_nome: exam.pdfName || "" }); setExamOpen(true); }}>Editar</Button><Button variant="outline" size="sm" className="border-red-200 text-red-600 hover:bg-red-50" onClick={() => void deletePatientExamRecord(patient.id, exam.id).then(async () => setExamRecords(await fetchPatientExamRecords(patient.id)))}>Excluir</Button></div></div><p className="mt-3 text-sm text-gray-700">{exam.result}</p>{exam.description && <p className="mt-2 text-sm text-gray-500">{exam.description}</p>}{exam.pdfName && <p className="mt-2 text-sm text-primary">Arquivo anexado: {exam.pdfName}</p>}</div>) : <p className="text-sm text-gray-500">Nenhum exame registrado.</p>}</CardContent></Card></TabsContent>
      ...
      <Dialog open={examOpen} onOpenChange={(open) => { setExamOpen(open); if (!open) { setEditingExamId(null); setExamMode("existing"); setExamForm({ selectedCatalogId: "", nome: "", categoria: "laboratorial", descricaoCatalogo: "", data_exame: "", status: "", resultado: "", observacoes: "", pdf_nome: "" }); } }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[720px]"><DialogHeader><DialogTitle>{editingExamId ? "Editar exame do paciente" : "Adicionar exame"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="grid grid-cols-2 gap-2"><Button type="button" variant={examMode === "existing" ? "default" : "outline"} onClick={() => setExamMode("existing")}>Selecionar existente</Button><Button type="button" variant={examMode === "new" ? "default" : "outline"} onClick={() => setExamMode("new")}>Criar novo no catálogo</Button></div>{examMode === "existing" ? <div className="space-y-2"><Label>Exame do catálogo</Label><Select value={examForm.selectedCatalogId} onValueChange={(value) => setExamForm((c) => ({ ...c, selectedCatalogId: value }))}><SelectTrigger><SelectValue placeholder="Selecione um exame" /></SelectTrigger><SelectContent>{examCatalog.map((exam) => <SelectItem key={exam.exame_id} value={String(exam.exame_id)}>{exam.nome}</SelectItem>)}</SelectContent></Select></div> : <div className="space-y-3 rounded-xl border p-4"><div className="space-y-2"><Label>Nome do novo exame</Label><Input value={examForm.nome} onChange={(e) => setExamForm((c) => ({ ...c, nome: e.target.value }))} /></div><div className="space-y-2"><Label>Descrição do catálogo</Label><Textarea rows={3} value={examForm.descricaoCatalogo} onChange={(e) => setExamForm((c) => ({ ...c, descricaoCatalogo: e.target.value }))} /></div></div>}<div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Data do exame</Label><Input type="date" value={examForm.data_exame} onChange={(e) => setExamForm((c) => ({ ...c, data_exame: e.target.value }))} /></div><div className="space-y-2"><Label>Status</Label><Input value={examForm.status} onChange={(e) => setExamForm((c) => ({ ...c, status: e.target.value }))} /></div></div><div className="space-y-2"><Label>Resultado</Label><Textarea rows={3} value={examForm.resultado} onChange={(e) => setExamForm((c) => ({ ...c, resultado: e.target.value }))} /></div><div className="space-y-2"><Label>Observações</Label><Textarea rows={3} value={examForm.observacoes} onChange={(e) => setExamForm((c) => ({ ...c, observacoes: e.target.value }))} /></div><div className="space-y-2"><Label>Upload de resultado</Label><Input type="file" accept=".pdf,image/*" onChange={(e) => setExamForm((c) => ({ ...c, pdf_nome: e.target.files?.[0]?.name || "" }))} />{examForm.pdf_nome && <p className="text-sm text-gray-500">Arquivo selecionado: {examForm.pdf_nome}</p>}</div></div><DialogFooter><Button variant="outline" onClick={() => setExamOpen(false)}>Cancelar</Button><Button onClick={() => void saveExam()}>Salvar</Button></DialogFooter></DialogContent></Dialog>
